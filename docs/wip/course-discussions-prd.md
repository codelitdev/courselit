...
5. Learners can participate only when they can access the course and lesson.
6. Product admins are users who can manage the product through existing product permissions.
7. Replies are stored and displayed as one-level replies under the original top-level comment. Replying to a reply adds the new reply to that same reply list.
8. The feature should use existing GraphQL patterns unless a REST endpoint is already the natural integration point for the touched surface.
...
2. Learners see the current lesson's discussion in a right sidebar on desktop and a full-screen panel on mobile.
3. Learners can create top-level comments, reply to comments or replies, like comments/replies, delete their own content, and report other participants' content.
...
2. As a learner, I can open the discussion panel while viewing a lesson and see comments for only that lesson.
3. As a learner, I can reply to a top-level comment or to an existing reply, and my reply appears in that comment's reply list.
4. As a learner, I can delete my own comment or reply without deleting the rest of the thread.
5. As a learner, I can report a comment or reply for admin review.
6. As a learner, I can like or unlike a comment or reply.
...
9. As a learner, I can open `/course/[slug]/[id]/discussions`, see lesson discussion counts, and jump into the relevant lesson discussion.
...
### Admin Discussion Overview

- Add a discussion overview under the product manage route so product admins can review discussions without learner enrollment.
- Recommended route: `/dashboard/product/[id]/manage/discussions`.
- `/dashboard/product/[id]/manage/discussions` is the full admin discussion browser for the product.
- In a future blog-comments release, this route will also show product-level comments for the managed product.
- The overview must be available to users with product management permissions even when they are not enrolled as learners.
- The overview lists all course lessons that have discussion activity, including lessons that are not currently dripped to a given learner.
- Admins can open a lesson's discussion from the overview and review comments/replies in a dashboard/admin UI.
- Admins cannot directly delete comments or replies from this overview.
- Admins can report comments/replies from this overview to send them into the report queue.
- Admin views must show deleted comments/replies with the original content and a clear deleted label.
- Admins can restore deleted comments/replies from report/moderation views.
- Admin discussion review must not require navigating through the public course viewer.
- This screen should include a reported-content button/link, matching the community manage screen pattern.
- Admins can use the reported-content entry point to navigate to reported discussion items.
- This admin overview is separate from learner-facing `/course/[slug]/[id]/discussions`, which remains filtered by the learner's access and drip state.
- Product admins can participate from the learner-facing course viewer only if they are enrolled in the course and have learner access to the lesson.
- Product management permissions alone do not grant learner-facing discussion participation.
...
- Learner-facing views must not return or render the original content of deleted comments/replies.
- Each visible comment and reply shows its like count and whether the current learner has liked it.
- Learners can like or unlike visible comments and replies.
...
- Admin discussion overview and report screens must also use pagination for lesson summaries, comments, replies, and reports.
...
- The page lists only lessons that are currently accessible to the learner under existing course access and drip rules.
- For accessible lessons, show lessons with at least one visible or deleted discussion item, plus counts.
- Do not show accessible lessons with zero discussion activity.
- Do not show discussion activity for lessons that have not dripped to the learner yet.
- Server logic must resolve the actor's accessible lesson IDs before querying discussion summaries/counts.
- Discussion summaries/counts must be queried only for that accessible lesson allowlist; do not query all lesson discussion counts and filter them client-side.
...
- Add a product moderation area under the product manage route, for example:
    - `/dashboard/product/[id]/manage/discussions/reports`
    - or another reports route linked from `/dashboard/product/[id]/manage/discussions`
- `/dashboard/product/[id]/manage/discussions/reports` is the report queue. It is not the primary discussion browser.
- The report queue should show only reported content and report workflow controls.
- General browsing, lesson-level discussion review, and admin-created reports happen in `/dashboard/product/[id]/manage/discussions`.
...
- Report and admin moderation views may show original deleted content, but must label it clearly as deleted.
- Report/moderation views must allow restoring deleted comments/replies through the same report status cycle.
...
One document per discussion target summary. This avoids expensive aggregation for learner `/discussions` and admin discussion overview screens.
...
- Learner `/discussions` can use `activityCountIncludingDeleted > 0` to show accessible lessons that contain only deleted placeholders, while visible counts continue to come from `commentsCount`, `repliesCount`, and `totalCount`.
- Learner `/discussions` must filter summaries by the server-resolved accessible lesson ID allowlist.
- Admin discussion overview can read summaries for all lessons in the managed course.
...
- Add or reactivate a subscription for a learner when they create a top-level comment or reply in a lesson discussion.
...
- Product admins are notification recipients through product management permissions, not through this subscriber collection unless they also participate as enrolled learners.
...
- discussions must be enabled for learner-facing write operations
...
- actor must have access to the target entity before reading or writing discussion content:
    - for `entityType = "lesson"`, enforce existing course enrollment, drip, visibility, and lesson access rules
- learner-facing write mutations must pass course discussion rate-limit checks
...
- lesson discussion summaries for `/discussions` must exclude lessons that are not currently accessible to the actor, including lessons blocked by drip rules
- `/discussions` summary generation must resolve accessible lesson IDs first and use them as a server-side query filter before reading discussion counts
- product admins can read and moderate all discussions for a managed product from dashboard/manage routes without learner enrollment
- product admins can create learner-facing comments/replies only when they are enrolled and pass the same lesson access checks as learners
...
- product admin report status updates that restore content must make the content visible to learners again and record `restoredBy` and `restoredAt`
...
- learners can like/unlike accessible, non-deleted comments and replies
- product admins can report any discussion content from the admin discussion browser
...
- For `entityType = "lesson"`, learner access must follow existing enrollment, visibility, drip, and lesson access rules.
- For `entityType = "product"`, learner-facing read/write APIs must reject access in this release because product-level comments are future-only.
...
- Learner-facing APIs must redact deleted comment/reply content and return only deleted placeholders.
...
- Use `@courselit/page-primitives` for every public learner-facing discussion UI surface outside `/dashboard` so controls inherit the active theme.
- Use existing course viewer visual language for learner surfaces.
...
- Admin discussion overview tests for non-enrolled product admins with product management permissions.
...
- Restore tests for report-gated admin restoration and learner visibility after restore.
...
- Add admin discussion overview and moderation route under product manage.
...
- Manually verify desktop and mobile course viewer flows.
...
- `entityType = "lesson"` validates product ownership, lesson membership, enrollment/drip/access rules, and enabled discussions.
...
- GraphQL logic tests cover pagination, deep-link target loading, create, reply-to-reply flattening with `parentReplyId`, delete, counter updates, and learner redaction of deleted content.
...
- Tenant and access checks are covered before learner UI begins.

### Task 6: Learner Discussion Panel
...
- Public learner UI uses `@courselit/page-primitives` and existing viewer visual language.
...
### Task 7: Learner All Discussions Page And Deep Links

Description: Add `/course/[slug]/[id]/discussions` and notification/hash-driven opening/highlighting behavior.

Acceptance criteria:

- The page lists only server-filtered accessible/dripped lesson summaries.
...
- Tests cover accessible lesson filtering, no future-lesson leakage, row navigation, panel opening, and highlight behavior.
...
### Task 9: Admin Discussion Overview

Description: Add `/dashboard/product/[id]/manage/discussions` as the product admin discussion browser.

Acceptance criteria:

- Product admins can browse all lesson discussion summaries for the product without learner enrollment.
- Admin views can show deleted content with clear labels.
- Admins can report content from the overview but cannot directly delete or restore there.
- The reported-content entry point links to the report queue.

Verification:

- Admin tests cover non-enrolled product admin access, tenant isolation, deleted-content visibility, and report-from-overview behavior.

Dependencies: Tasks 4-5.

Estimated scope: Medium.

### Task 10: Admin Report Queue And Moderation
...
- Moderation tests cover status transitions, counter changes, restore behavior, duplicate accepted reports, permission checks, and learner visibility changes.
...
- Learner can create, reply, like, report, delete, browse `/discussions`, and follow deep links.
- Product admin can enable discussions, browse discussions, report content, moderate reports, and restore content.
...
- Run `pnpm lint`.
- Run `pnpm prettier`.
- Manual smoke test desktop and mobile learner/admin flows.
...
- When enabled, every accessible lesson can host a lesson-specific discussion for `entityType = "lesson"`.
- Replies are always flattened under the top-level comment.
- Learners can like and unlike visible comments and replies, and like state/counts update consistently.
- `/course/[slug]/[id]/discussions` lists lesson discussion activity and deep-links correctly.
- `/course/[slug]/[id]/discussions` never reveals discussion activity for lessons that have not dripped to the learner.
...
- Learners can report content and delete their own content.
- Product admins can review all discussions and report content from the admin discussion browser without learner enrollment.
