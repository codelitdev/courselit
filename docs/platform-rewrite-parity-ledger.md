...
- **Target implementation/evidence:**
  ...
  metadata, serves private package files through the storefront app, exposes SCORM 1.2/2004
  ...
  correct answers from learner product responses. The storefront app now also has a
  ...
  `apps/storefront/components/layout/course-viewer-sidebar.tsx`: About and Discussions,
...
- **Target implementation/evidence:** `apps/api/src/auth/options.ts` configures
  ...
  `apps/storefront/components/public-login-block.tsx` and
  `apps/storefront/components/checkout-login-form.tsx` use that realm directly.
...
- **Target implementation/evidence:** the split storefront app mounts the aggregate feed
  at `/dashboard`, filtered space feeds at `/dashboard/s/<spaceId>`, and products at
  `/dashboard/products`. `apps/storefront/components/dashboard/learner-products.tsx`
...
- **Main source of truth:**
  ...
  worktree.
- **Target implementation/evidence:** `apps/storefront/components/lesson-viewer.tsx`
  uses `@frontlit/text-editor` for ProseMirror rendering and delegates embed
  lessons to `apps/storefront/components/embed-viewer.tsx`. YouTube URLs render in
...
| Public/browser       | `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_BASE_PATH`, public CAPTCHA/Turnstile keys                                                             | Split by admin and storefront app; prohibit accidental secret promotion                           |
