...
import { SIGN_IN, SIGN_OUT, NETWORK_ACTION, PROFILE_AVAILABLE, PROFILE_CLEAR, SITEINFO_AVAILABLE, AUTH_CHECKED, SET_MESSAGE, CLEAR_MESSAGE, THEME_AVAILABLE, SET_ADDRESS, TYPEFACES_AVAILABLE, CONFIG_AVAILABLE, } from "./action-types";
        ...
        case SITEINFO_AVAILABLE:
            try {
                return {
                    ...
                    paymentMethod: action.siteinfo.paymentMethod ||
                        initialState.siteinfo.paymentMethod,
                    stripeKey: action.siteinfo.stripeKey ||
                        initialState.siteinfo.stripeKey,
                    codeInjectionHead: decode(action.siteinfo.codeInjectionHead) ||
                        initialState.siteinfo.codeInjectionHead,
                    codeInjectionBody: decode(action.siteinfo.codeInjectionBody) ||
                        initialState.siteinfo.codeInjectionBody,
                    mailingAddress: action.siteinfo.mailingAddress ||
                        initialState.siteinfo.mailingAddress,
                    hideCourseLitBranding: action.siteinfo.hideCourseLitBranding ||
                        initialState.siteinfo.hideCourseLitBranding,
                    razorpayKey: action.siteinfo.razorpayKey ||
...
*/
function typefacesReducer(state = initialState.typefaces, action) {
    switch (action.type) {
...
function configReducer(state = initialState.config, action) {
    switch (action.type) {
...
const reducer = (state = initialState, action) => {
    if (action.type === HYDRATE) {
        const nextState = Object.assign(Object.assign({}, state), action.payload);
