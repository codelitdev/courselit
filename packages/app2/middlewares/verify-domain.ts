...
const getDomainBasedOnSubdomain = async (subdomain: string): Promise<Domain | null> => {
  return await DomainModel.findOne({ name: subdomain, deleted: false });
};

const getDomainBasedOnCustomDomain = async (customDomain: string): Promise<Domain | null> => {
  return await DomainModel.findOne({ customDomain, deleted: false });
};

const getDomain = async ({ hostName, domainName }: { hostName: string, domainName: string }): Promise<Domain | null> => {
...
export default async function verifyDomain (req: ApiRequest, res: NextApiResponse): Promise<void> {
    if (process.env.MULTITENANT === "true") {
        ...
        let domain = await DomainModel.findOne({ name: domainNameForSingleTenancy });
