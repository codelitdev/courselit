...
import {
  ...
  readLegacyNotificationExport,
} from "../src/migrations/notifications.ts";
...
if (!inputPath || !databaseUrl) {
  throw new Error(
    "usage: DATABASE_URL=... bun scripts/import-notifications.mjs --input notifications.json [--apply]",
  );
}
...
try {
  ...
  const result = await importLegacyNotifications(db, {
    exportData: readLegacyNotificationExport(inputPath),
    clock: systemClock,
    mode,
    sourceSystem,
  });
