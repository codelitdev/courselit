...
import {
  importLegacyDomains,
  readLegacyDomainExport,
} from "../src/migrations/domains.ts";
...
if (!inputPath || !databaseUrl) {
  throw new Error(
    "usage: DATABASE_URL=... bun scripts/import-domains.mjs --input domains.json [--apply]",
  );
}
...
try {
  ...
  const result = await importLegacyDomains(db, {
    records: readLegacyDomainExport(inputPath),
    clock: systemClock,
    mode,
    sourceSystem,
  });
