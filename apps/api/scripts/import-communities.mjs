...
import {
  importLegacyCommunities,
  readLegacyCommunityExport,
} from "../src/migrations/communities.ts";
...
if (!inputPath || !databaseUrl) {
  throw new Error(
    "usage: DATABASE_URL=... bun scripts/import-communities.mjs --input communities.json [--apply]",
  );
}
...
try {
  ...
  const result = await importLegacyCommunities(db, {
    exportData: readLegacyCommunityExport(inputPath),
    clock: systemClock,
    mode,
    sourceSystem,
  });
