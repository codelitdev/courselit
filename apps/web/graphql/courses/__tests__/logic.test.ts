...
describe("product discussion comment and reply logic", () => {
    ...
    it("lets product admins browse original deleted discussion content and create reports", async () => {
        const adminCtx = {
            subdomain: testDomain,
            user: productAdminUser,
            address: "",
...
describe("getCourse", () => {
    ...
    it("allows course managers to preview unpublished courses", async () => {
        const course = await CourseModel.create({
            domain: testDomain._id,
...
describe("public API product read helpers", () => {
    ...
    it("returns courses for course:manage permission user", async () => {
        const courseManageUser = await UserModel.create({
            domain: testDomain._id,
            userId: helperId("manage-user"),
            email: `${helperId("manage")}@example.com`,
            name: "Course Manager User",
            permissions: [constants.permissions.manageCourse],
            active: true,
            unsubscribeToken: helperId("unsubscribe-manage"),
            purchases: [],
        });

        const course = await CourseModel.create({
            domain: testDomain._id,
            courseId: helperId("course-1"),
            title: "Course 1",
        ...
        expect(products.map((p) => p.courseId)).toContain(course.courseId);
    });
    ...
    it("restricts overview metrics to owned products for course:manage users", async () => {
        ...
        // 1. Should succeed for own course when calling getActivities with entityId
        await expect(
            getActivities({
                ctx,
        ...
        // 2. Should reject for another course (not owned by the user)
        await expect(
            getActivities({
                ctx,
                type: CommonConstants.ActivityType.PURCHASED,
        ...
        // 3. Should succeed and return aggregated metrics (where entityId is undefined) for own courses only
        ...
        // The count should reflect the purchases for owned courses only (cost = 100, not 150)
