...
import {
    ...
    repairOrphanCourseDiscussionCommunities,
} from "../logic";
...
describe("course discussions – deleteCourse cascade", () => {
    ...
    it("orphan repair hard-deletes orphaned course-linked discussion communities", async () => {
        const orphanCommunity = await CommunityModel.create({
            domain: testDomain._id,
            communityId: casId("orphan-community"),
            name: "Orphan Discussion",
            slug: casId("orphan-community"),
            pageId: casId("orphan-community"),
            courseId: casId("missing-course"),
        ...
        const post = await CommunityPostModel.create({
            domain: testDomain._id,
            postId: casId("orphan-post"),
            communityId: orphanCommunity.communityId,
            userId: ownerUser.userId,
            title: "Orphan post",
            content: "",
            category: "General",
            lessonId: casId("orphan-lesson"),
        });
        const comment = await CommunityCommentModel.create({
            domain: testDomain._id,
            commentId: casId("orphan-comment"),
            communityId: orphanCommunity.communityId,
            postId: post.postId,
            userId: ownerUser.userId,
        ...
        await CommunityReportModel.create({
            domain: testDomain._id,
            reportId: casId("orphan-report"),
            communityId: orphanCommunity.communityId,
            contentId: post.postId,
            type: CommonConstants.CommunityReportType.POST,
            reason: "Orphan report",
            userId: ownerUser.userId,
        });
        await CommunityPostSubscriberModel.create({
            domain: testDomain._id,
            subscriptionId: casId("orphan-subscriber"),
            postId: post.postId,
            userId: ownerUser.userId,
            subscription: true,
        });
        await MembershipModel.create({
            domain: testDomain._id,
            membershipId: casId("orphan-membership"),
            sessionId: casId("orphan-session"),
            userId: ownerUser.userId,
            entityId: orphanCommunity.communityId,
            entityType: CommonConstants.MembershipEntityType.COMMUNITY,
            paymentPlanId: casId("orphan-plan"),
            status: CommonConstants.MembershipStatus.ACTIVE,
            role: CommonConstants.MembershipRole.COMMENT,
        });
        await PaymentPlanModel.create({
            domain: testDomain._id,
            planId: casId("orphan-plan"),
            name: "Orphan Plan",
            type: CommonConstants.PaymentPlanType.FREE,
            userId: ownerUser.userId,
            entityId: orphanCommunity.communityId,
        ...
        await NotificationModel.create({
            domain: testDomain._id,
            notificationId: casId("orphan-notification"),
            userId: ownerUser.userId,
            forUserId: ownerUser.userId,
            activityType:
                CommonConstants.ActivityType.COMMUNITY_COMMENT_CREATED,
        ...
        const result = await repairOrphanCourseDiscussionCommunities({
            domain: testDomain._id,
            batchSize: 2,
        });
        ...
        await expect(
            PaymentPlanModel.countDocuments({
                domain: testDomain._id,
                entityId: orphanCommunity.communityId,
            }),
        ).resolves.toBe(0);
        await expect(
            NotificationModel.countDocuments({
                domain: testDomain._id,
                "metadata.communityId": orphanCommunity.communityId,
            }),
        ).resolves.toBe(0);
        await expect(
            ActivityModel.countDocuments({
