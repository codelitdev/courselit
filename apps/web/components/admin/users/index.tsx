...
const UsersManager = ({
  ...
  profile,
    ...
    const getUserType = (user: User) => {
        ...
        const hasAudiencePermission = checkPermission(user.permissions, [permissions.enrollInCourse]);
            ...
            <Grid container direction="column" spacing={2}>
                            ...
                            <Table aria-label="Users">
                                <TableHead>
                                    <TableRow>
                                                ...
                                                <Grid item>
                                                    <FormControl sx={{ minWidth: 120 }}>
                                                        <InputLabel
                                                            ...
                                                            id="type-select"
                                                            shrink>{USER_FILTER_PERMISSION}</InputLabel>
                                                        <Select
                                                            ...
                                                            labelId="type-select"
                                                            size="small">
                                                            <MenuItem value="">{USER_TYPE_ALL}</MenuItem>
                                                            <MenuItem value={USER_TYPE_AUDIENCE}>{USER_TYPE_AUDIENCE}</MenuItem>
                                                            <MenuItem value={USER_TYPE_TEAM}>{USER_TYPE_TEAM}</MenuItem>
                                                        </Select>
                                                    </FormControl>
                                                </Grid>
                                                <Grid item>
                                        ...
                                        <TableCell align="right">{USER_TABLE_HEADER_JOINED}</TableCell>
                                        <TableCell align="right">{USER_TABLE_HEADER_LAST_ACTIVE}</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {users.map(user => (
                                                    ...
                                                    <Grid item direction="column">
                                                        <Grid item>
                                                            <Link href={`/dashboard/users/${user.userId}`}>
                                                                <MuiLink color="inherit" variant="body1">
                                                                    <b>
                ...
                {users.length > 0 && (
                    <Grid item container justifyContent="center">
                        <Grid item>
                            <Button variant="outlined" onClick={() => setUsersPaginationOffset(usersPaginationOffset + 1)}>
                                {LOAD_MORE_TEXT}
                            </Button>
                        </Grid>
                    </Grid>
