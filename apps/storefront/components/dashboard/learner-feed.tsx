...
export function LearnerFeed({
  spaceId,
  embeddedPostDetails = false,
  embeddedPostId,
  onEmbeddedPostIdChange,
}: {
  spaceId?: string;
  embeddedPostDetails?: boolean;
  ...
  const [internalEmbeddedPostId, setInternalEmbeddedPostId] = useState<string | null>(
    null,
  );
  ...
  const postableSpaces = useMemo(
    () =>
      spaces.filter(
        (space) =>
          (!requestedSpaceId || space.id === requestedSpaceId) &&
          (space.canPost ?? space.whoCanPost === "members"),
      ),
    [requestedSpaceId, spaces],
  );

  useEffect(() => {
    setInternalEmbeddedPostId(null);
    onEmbeddedPostIdChange?.(null);
  }, [onEmbeddedPostIdChange, requestedSpaceId]);
  ...
  const selectedEmbeddedPostId =
    embeddedPostId === undefined ? internalEmbeddedPostId : embeddedPostId;

  function selectEmbeddedPost(postId: string | null) {
    if (embeddedPostId === undefined) setInternalEmbeddedPostId(postId);
    onEmbeddedPostIdChange?.(postId);
  }

  function openPost(post: FeedPost, reply = false) {
    const targetSpaceId = post.spaceId ?? post.space?.id;
    if (!targetSpaceId) return;
    if (embeddedPostDetails && targetSpaceId === requestedSpaceId) {
      selectEmbeddedPost(post.id);
      return;
    }
  ...
  if (embeddedPostDetails && requestedSpaceId && selectedEmbeddedPostId) {
    return (
      <div className="grid w-full gap-4">
        <LearnerSpacePost
          spaceId={requestedSpaceId}
          postId={selectedEmbeddedPostId}
          embedded
        />
      </div>
    );
  ...
  return (
    <div className="mx-auto grid w-full max-w-4xl gap-7">
      ...
      {loading ? (
        <CourseLitLoading label="Loading feed…" className="min-h-64" />
      ) : (
      ...
      <LearnerPostComposerDialog
        ...
        destinationLocked={Boolean(requestedSpaceId)}
