 ...
 */
const move = (file, path) => new Promise((resolve, reject) => {
  file.mv(path, (err) => {
...
router.post('/', async (req, res) => {
  ...
  const data = req.body
  if (!data.title) return res.status(400).json({ message: responses.title_is_required })
  if (!req.files || !req.files.file) return res.status(400).json({ message: responses.file_is_required })
